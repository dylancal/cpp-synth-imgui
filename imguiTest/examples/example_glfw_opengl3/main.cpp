...
private:
    int paCallbackMethod(const void* inputBuffer, void* outputBuffer,
        ...
        for (std::size_t i = 0; i < framesPerBuffer; i++) {
            *out++ = amplitude.load(std::memory_order_relaxed) * (m_oscA.ps.amp.load(std::memory_order_relaxed) * (half_f_add_one(2 * m_lfoA.lfo_amp * m_lfoA.interpolate_amp())) * m_oscA.interpolate_left() +
                m_oscB.ps.amp.load(std::memory_order_relaxed) * half_f_add_one(2 * m_lfoB.lfo_amp * m_lfoB.interpolate_amp()) * m_oscB.interpolate_left() +
                m_oscC.ps.amp.load(std::memory_order_relaxed) * half_f_add_one(2 * m_lfoC.lfo_amp * m_lfoC.interpolate_amp()) * m_oscC.interpolate_left());
            *out++ = amplitude.load(std::memory_order_relaxed) * (m_oscA.ps.amp.load(std::memory_order_relaxed) * (half_f_add_one(2 * m_lfoA.lfo_amp * m_lfoA.interpolate_amp())) * m_oscA.interpolate_right() +
                m_oscB.ps.amp.load(std::memory_order_relaxed) * half_f_add_one(2 * m_lfoB.lfo_amp * m_lfoB.interpolate_amp()) * m_oscB.interpolate_right() +
                m_oscC.ps.amp.load(std::memory_order_relaxed) * half_f_add_one(2 * m_lfoC.lfo_amp * m_lfoC.interpolate_amp()) * m_oscC.interpolate_right());
...
int main(int, char**) {
   ...
   /* ImGui::StyleColorsDark();*/ // not needed since we are using a custom theme
    ...
    const char* waveforms[] = { "Sawtooth", "Sine", "Square"};
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (auto& oscpair : st.oscillators) {
            ...
            if (ImGui::CollapsingHeader("General Settings", ImGuiTreeNodeFlags_DefaultOpen))
            {
                ...
                // pitch fine tune
                if (ImGui::DragFloat("Left Phase Increment", gui_left_phase_incs[osc_idx], 0.005f, 1, 20, "%f")) 
                    gui_updated = true;
                if (ImGui::Combo("R-Note", (int*)&osc->ps.current_note_right, notes, IM_ARRAYSIZE(notes))) {
                    *gui_right_phase_incs[osc_idx] = freqs[osc->ps.current_note_right];
                    gui_updated = true;
                }
                if (ImGui::DragFloat("Right Phase Increment", gui_right_phase_incs[osc_idx], 0.005f, 1, 20, "%f"))
                    gui_updated = true;
            }
